...
function doGet(e) {
  try {
    ...
    if (row) {
      ...
      const newRow = [charId, 5, 2, 5, 5, "[]", "", "[]", "20,30"];
...
function askSecretOracle(userPrompt) {
  ...
  try {
    ...
    return res.getResponseCode() === 200 ? data.candidates[0].content.parts[0].text : "VOID LINK ERROR: " + (data.error ? data.error.message : "SIGNAL LOST");
  } catch (e) { return "SIGNAL LOST: SYSTEM CRASH."; }
}
